...
// Payments List tab — same Payment Logs data as the Supplier Payments tab,
// but grouped by PO (one card per PO, terms nested inside). Only returns
// terms that haven't been settled yet (no Bank/Check/Amount stamped).
function getPaymentTermsQueue() {
  try {
    // Make sure Payment Logs has term rows for every PO before reading.
    const poSheet = SS.getSheetByName(SHEETS.PO_QUEUE);
    if (poSheet && poSheet.getLastRow() >= 2) {
      const poIds = poSheet.getRange(2, 2, poSheet.getLastRow() - 1, 1).getValues();
      const seenSeed = {};
      for (let i = 0; i < poIds.length; i++) {
        const id = (poIds[i][0] || "").toString().trim();
        if (id && !seenSeed[id.toLowerCase()]) {
          seenSeed[id.toLowerCase()] = true;
          ensurePaymentLogRowsForPO_(id);
        }
      }
    }

    // Build a PO-display-id → { project, supplier, items[] } index from PO_QUEUE
    // so we can render the project label and item list under each card.
    const poInfo = {};
    if (poSheet && poSheet.getLastRow() >= 2) {
      const poData = poSheet.getRange(2, 1, poSheet.getLastRow() - 1, 13).getValues();
      for (let i = 0; i < poData.length; i++) {
        const rawId = (poData[i][1] || "").toString().trim();
        if (!rawId) continue;
        const dispKey = poDisplayId_(rawId).toLowerCase();
        if (!poInfo[dispKey]) {
          poInfo[dispKey] = {
            mrfId: rawId,
            project: (poData[i][2] || "").toString(),
            supplier: (poData[i][9] || "").toString(),
            items: []
          };
        }
        const itemName = (poData[i][5] || "").toString();
        if (itemName && poInfo[dispKey].items.indexOf(itemName) === -1) {
          poInfo[dispKey].items.push(itemName);
        }
      }
    }

    const payLogs = getOrCreatePaymentLogsSheet_();
    if (payLogs.getLastRow() < 2) return [];
    const payData = payLogs.getRange(2, 1, payLogs.getLastRow() - 1, PAYMENT_LOG_HEADERS.length).getValues();

    const groups = {};
    for (let i = 0; i < payData.length; i++) {
      const po = (payData[i][0] || "").toString().trim();
      if (!po) continue;

      // Skip terms that already have a real payment recorded.
      const bank   = (payData[i][6] || "").toString().trim();
      const check  = (payData[i][7] || "").toString().trim();
      const amount = parseFloat(payData[i][8]) || 0;
      if (bank && check && amount > 0) continue;

      const key = po.toLowerCase();
      const info = poInfo[key] || { mrfId: po, project: "", supplier: (payData[i][3] || "").toString(), items: [] };

      if (!groups[key]) {
        groups[key] = {
          mrfId: po,                  // Already in display form (PO-xxxx)
          project: info.project,
          supplier: info.supplier || (payData[i][3] || "").toString(),
          termsList: []
        };
      }

      // Render the due date the way the UI expects (YYYY-MM-DD parsed, or pass-through).
      let dueStr = "";
      const rawDue = payData[i][5];
      if (rawDue) {
        const d = new Date(rawDue);
        dueStr = isNaN(d.getTime()) ? rawDue.toString() : formatFullDate(d);
      }

      groups[key].termsList.push({
        termDesc: (payData[i][1] || "").toString(),
        termDate: dueStr,
        items: info.items,
        totalAmount: parseFloat(payData[i][4]) || 0,
        rowIndices: [i + 2]   // sheet row in Payment Logs — used by markPaymentDeposited
      });
    }

    const out = [];
    for (const k in groups) out.push(groups[k]);
    return out.reverse();
  } catch (e) {
    return [];
  }
